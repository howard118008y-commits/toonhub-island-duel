...
const patterns = new Set(Object.keys(MOTIFS));
...
const colorKeys = ['bg', 'panel', 'ink', 'accent'];

export function readTheme(data) {
  if (!data || data.type !== 'island-theme' || !Number.isInteger(data.cardId) || data.cardId < 1 || data.cardId > 24) return null;
  if (!patterns.has(data.pattern) || colorKeys.some(key => typeof data[key] !== 'string' || !/^#[\da-f]{6}$/i.test(data[key]))) return null;
  return { cardId: data.cardId, pattern: data.pattern, ...Object.fromEntries(colorKeys.map(key => [key, data[key]])) };
}

export function installThemeBridge() {
  document.documentElement.style.setProperty('--motif', patternImage('coffee', '#843B25'));
  window.addEventListener('message', event => {
    if (event.source !== window.parent || event.origin !== location.origin) return;
    const theme = readTheme(event.data);
    if (!theme) return;
    const root = document.documentElement;
    colorKeys.forEach(key => root.style.setProperty(`--theme-${key}`, theme[key]));
    root.style.setProperty('--motif', patternImage(theme.pattern, theme.accent));
    root.dataset.pattern = theme.pattern;
    root.dataset.themeCard = String(theme.cardId);
    document.querySelector('meta[name="theme-color"]')?.setAttribute('content', theme.bg);
  });
  const ready = () => {
    if (window.parent !== window) window.parent.postMessage({ type: 'island-theme-ready' }, location.origin);
  };
